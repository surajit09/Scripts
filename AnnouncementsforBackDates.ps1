...
$DocLibArray = "Updates","Buying Carpets","Buying Beds","Buying Hardflooring", "Marketing Bulletin","Concept Stores","POS Packs","Operations Bulletin", "Peak Trade Communications","News Letters","Operations Procedures","HR Policies and Procedures","Health and Safety policies", "Executive Memos", "Facilities Policies", "H&S Manual (Check Lists)","Risk Assessments", "Training",  "IT Policies" ,"Customer Service","Buying Policies","Finance Policies", "Company Announcements", "eCommerce Policies","Marketing Policies","Warehouse Policies","Templates", "Regional Documents"
		...
		foreach ($listItem in $list.Items)
		{
			...
			if($creationDate.ToShortDateString() -eq $toDay.ToShortDateString())
			{
				...
				if($count -eq 0)
				{
					...
					$DocumentUrl="<a href='"+$web.url+"/"+$DocUrl+"'>" + " Click Here" +"</a>"
